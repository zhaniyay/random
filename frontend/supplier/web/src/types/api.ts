import type { LinkStatusType, OrderStatusType, ComplaintStatusType } from '../models/enums';

// API Response Types
export interface TokenOut {
  access_token: string;
}

export interface RegisterIn {
  email: string;
  password: string;
  role: 'OWNER' | 'MANAGER' | 'SALES';
  supplier_name: string;
}

export interface ConsumerInfo {
  id: number;
  name: string;
}

export interface SupplierInfo {
  id: number;
  name: string;
}

export interface LinkOut {
  id: number;
  supplier_id: number;
  consumer_id: number;
  status: LinkStatusType;
  created_at: string;
  consumer?: ConsumerInfo;
  supplier?: SupplierInfo;
}

export interface ProductCreateIn {
  name: string;
  unit: string;
  price: number;
  moq: number;
  stock: number;
}

export interface ProductUpdateIn {
  name?: string;
  unit?: string;
  price?: number;
  moq?: number;
  stock?: number;
}

export interface ProductOut {
  id: number;
  supplier_id: number;
  name: string;
  unit: string;
  price: number;
  moq: number;
  stock: number;
}

export interface OrderItemOut {
  product_id: number;
  quantity: number;
  price: number;
  line_total: number;
}

export interface OrderOut {
  id: number;
  supplier_id: number;
  consumer_id: number;
  status: OrderStatusType;
  created_at: string;
  comment?: string;
  total_amount: number;
  items: OrderItemOut[];
}

export interface MessageCreateIn {
  body: string;
  link_id?: number;
  order_id?: number;
  message_type?: 'TEXT' | 'IMAGE' | 'FILE' | 'AUDIO';
}

export interface MessageOut {
  id: number;
  sender_user_id: number;
  body: string;
  created_at: string;
  link_id?: number;
  order_id?: number;
  message_type: 'TEXT' | 'IMAGE' | 'FILE' | 'AUDIO';
  file_name?: string;
  file_path?: string;
  file_size?: number;
  mime_type?: string;
}

export interface ComplaintOut {
  id: number;
  order_id: number;
  consumer_id: number;
  supplier_id: number;
  description: string;
  status: ComplaintStatusType;
  created_at: string;
}

// Incident types removed - now using complaints for both consumer and supplier issues

export interface User {
  id: number;
  email: string;
  role: 'OWNER' | 'MANAGER' | 'SALES';
  supplier_id: number;
}

